...
##############################
# Test bluetooth transfer, by Lile Wang
...
def PageEntered(update_time=True):
    ...
    # print >>sys.stdout, 'CurrentPage_%d' % Pcurrent;
    # sys.stdout.flush()
...
def TransitionTo(page, allow_transition=True):
    ...
    try:
        bluetooth_transfer.send( Pcurrent );
    except:
        pass;
    #
...
if __name__ == "__main__":
    try:
        if '-bt' in sys.argv[ 1 ]:
            bluetooth_transfer = bt_trans( sys.argv[ 2 ] );
            bluetooth_transfer.start(  );
            ParseOptions(sys.argv[3:])
        else:
